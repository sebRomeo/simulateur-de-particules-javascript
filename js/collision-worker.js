...
let config, particleCouples, broadPhaseMaxHorizontalDistance, gravityValue;

onmessage = function (e) {
    const [type, ...data] = e.data;
    switch (type) {
        case 'updateCollisions':
            updateCollisions(data[0]);
            break;
        case 'registerGlobals':
            config = data[0]
            particleCouples = data[1]
            broadPhaseMaxHorizontalDistance = data[2]
            gravityValue = data[3]
            nbParticules = data[4]
            break;
    }
...
function updateCollisions(Particules) {
    ...
    for (i1 = 0; i1 < nbParticules; i1++) {
        ...
        for (i2 = checked; i2 < nbParticules; i2++) {
            ...
            if (Math.abs(P2.position.x - P1.position.x) < broadPhaseMaxHorizontalDistance && Math.abs(P2.position.y - P1.position.y) < broadPhaseMaxHorizontalDistance) {
                ...
                P1.speed = Vector.speed(P1.force);
                P2.speed = Vector.speed(P2.force);
    ...
    if (!P1.collisionChecked && !P2.collisionChecked && distance < maxCollisionDistance) {
        ...
        for (; step > 0; step--) {
            ...
            if (Math.hypot(p2Xmp1X, p2Ymp1Y) <= friend.doubleRadius) {
                ...
                P1.force = Vector.scale(P1.force, config.restitution.value)
                P2.force = Vector.scale(P2.force, config.restitution.value)

                P1.position = Vector.add(P1.position, {
                    x: (P1.force.x / nbSteps) * step,
                    y: (P1.force.y / nbSteps) * step
                });
                P2.position = Vector.add(P2.position, {
