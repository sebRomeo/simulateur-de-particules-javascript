    ...
    for (i1 = 0; i1 < nbParticules; i1++) {
        ...
        for (i2 = checked; i2 < nbParticules; i2++) {
            ...
            if (!config.disableGravity) {
                ...
                const gravityValue = getGravityValue(distance);//(1 / distance) * config.gravity.value// 1000 * config.gravity.value * ((gravitationalConstant * P1.mass * P2.mass) / distSquared)
